In this section we have covered the below topics:

1) Create Gatway server: 
2) add gatewayserver.yml in git config of github
3) Add routes in gateway main class with two headers :
	X-Response-Time -> for time taken 
	bank-correlation-id -> for tracing between servicesbank-correlation-id is created in gatway when any new request is land to gateway
	
the bank-correlation-id is created in gatway when any new request is land to gateway. by using this we can trace particular request from all 
services.

 

1) update accounts service with bank-correlation-id as RequestHeader in microservice communication controller.
2) updated  card service with bank-correlation-id as RequestHeader in microservice communication controller.
